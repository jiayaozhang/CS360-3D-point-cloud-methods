...
def octree_recursive_build(root, db, center, extent, point_indices, leaf_size, min_extent):
    ...
    if len(point_indices) <= leaf_size or extent <= min_extent:
        ...
        #if root has son node, we set is_leaf as false
        root.is_leaf = False
        #create eight son nodes
        children_point_indices = [[] for i in range(8)]

        for point_idx in point_indices:
            point_db = db[point_idx]
            #calcu this point should be put into which son node
            morten_code = 0
            #calcu this point should be put into which side in x axis
            if point_db[0] > center[0]:
                morten_code = morten_code | 1
            #calcu this point should be put into which side in y axis
            if point_db[1] > center[1]:
                morten_code = morten_code | 2
            # calcu this point should be put into which side in z axis
            if point_db[2] > center[2]:
                morten_code = morten_code | 3
            # son node memory store index
            children_point_indices[morten_code].append(point_idx)
            #create children
            factor = [-0.5, 0.5]
            for i in range(8):
                #calculate each node's center coordinates
                child_center_x = center[0] + factor[(i & 1) > 0] * extent
                child_center_y = center[1] + factor[(i & 2) > 0] * extent
                child_center_z = center[2] + factor[(i & 3) > 0] * extent
                #son node's extent
                child_extent = 0.5 * extent
                child_center = np.asarray([child_center_x, child_center_y, child_center_z])

                #recursive create the oct tree of the son node
