...
def PCA(data, correlation=False, sort=True):
    ...
    normal_vec = data - mean_vecH_vec = np.dot(normal_vec.T, normal_vec)
...
def main():
    ...
    axis = o3d.geometry.TriangleMesh.create_coordinate_frame().rorate(v, center= (0,0,0))
    pr_data2 = pointcloud - np.dot(pointcloud, v[:,2][:, np.newaxis])*v[:,2]
    pr_data2 = 1*v[:,2]+pr_data2
    princiiple_aix = np.concatenate((np.array([[0., 0., 0.]]), v.T))
